...
curl --fail --silent --show-error --location     "$raw_base/party/index.html"     -o "$work_dir/index.html"

curl --fail --silent --show-error --location     "$raw_base/ops/party/userspace-party.caddy"     -o "$candidate_site"

grep -Fq '<title>userspace.party</title>' "$work_dir/index.html"     || fail "downloaded landing page failed identity check"
grep -Fq 'userspace.party {' "$candidate_site"     || fail "downloaded Caddy site failed identity check"

install -d -m 0755 -o root -g root "$base/releases"
install -d -m 0755 -o root -g root "$release_dir"
install -m 0644 -o root -g root "$work_dir/index.html" "$release_dir/index.html"
